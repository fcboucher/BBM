...
for (i in 1:length(tree$edge.length)){
	proptemp= ConvProp_bounds(X= VectorPos_bounds(trait[tree$edge[i,1]],Npts,bounds),t=tree$edge.length[i],dCoeff,dMat,bounds) # propagate the trait forward in time
